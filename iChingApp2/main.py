...
# clrBtn = Button(window, command=lambda:txtGua.delete(0.0, END))
clrBtn = Button(window)
exitBtn = Button(window, command=window.quit)

# layout design
imgBtn0.grid(row = 1, column = 1)
imgBtn1.grid(row = 1, column = 2)

txtGua.grid(row=2, column=1, columnspan=2, padx=(5, 2))

runBtn.grid(row = 3, column=1, columnspan=2)

# soundBtn.grid(row = 3, column=2)

clrBtn.grid(row = 4, column= 1)
exitBtn.grid(row = 4, column= 2)

# properties
window.title('易经占卦-文字版')
# window.geometry('300x600')
window.resizable(0, 0)

runBtn.configure(text = "卜卦", width=26)
# soundBtn.configure(text="Listen", width=6)
clrBtn.configure(text="清除", width=6)
exitBtn.configure(text="退出", width=6)
...
clrBtn.configure(command=clearGua)
